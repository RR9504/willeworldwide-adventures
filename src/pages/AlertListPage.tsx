...
const alertConfig: Record<AlertType, { title: string; description: string }> = {
  unpaid: {
    title: 'Inväntar betalning',
    description: 'Alla deltagare som ännu inte betalat sin resa.',
  },
  'missing-presentation': {
    title: 'Saknar "Lär känna"-svar',
    description: 'Deltagare som inte fyllt i sitt presentationsformulär.',
  },
  'low-spots': {
    title: 'Resor med få platser kvar',
    description: 'Publicerade resor med 10 eller färre platser kvar.',
  },
};
      ...
      <main className="container flex-1 py-8">
        ...
        {alertType === 'low-spots' && (
          <div className="space-y-3">
            {(grouped as { trip: typeof mockTrips[number]; spotsLeft: number; regCount: number }[]).map(({ trip, spotsLeft, regCount }) => (
              <Link
                key={trip.id}
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  <span className="inline-flex items-center rounded-full bg-orange-100 px-2.5 py-1 text-sm font-medium text-orange-700">
            ...
            {grouped.length === 0 && (
              <div className="py-12 text-center text-muted-foreground">Inga resor med få platser kvar just nu.</div>
            )}
          </div>
        )}

        {/* Unpaid / Missing presentation view */}
        {alertType !== 'low-spots' && (
          <div className="space-y-6">
            {(grouped as { trip: typeof mockTrips[number]; regs: typeof mockRegistrations }[]).map(({ trip, regs }) => (
              <Card key={trip.id}>
                <CardHeader>
                  <div className="flex items-center gap-4">
                    <img src={trip.image_url} alt={trip.title} className="h-10 w-10 rounded-md object-cover" />
                    <div className="flex-1">
                      <CardTitle className="text-base">
                        <Link to={`/dashboard/resor/${trip.id}`} className="hover:underline">
                          {trip.title}
                        </Link>
                      </CardTitle>
                                ...
                                <TableCell>
                                  <span
                                    ...
                                    onClick={e => {
                                      ...
                                      // Using inline toast import would be heavier, just use alert-style feedback
            ...
            {grouped.length === 0 && (
              <div className="py-12 text-center text-muted-foreground">
                {alertType === 'unpaid' ? 'Alla har betalat!' : 'Alla har fyllt i sina svar!'}
              </div>
            )}
          </div>
        )}
